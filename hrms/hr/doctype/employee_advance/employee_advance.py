...
import hrms
from hrms.hr.hr_custom_function import get_approver
...
def get_permission_query_conditions(user):
	...
	if user == "Administrator" or "System Manage" in user_roles:
		return
	...
	return """(
		...
		or
		(`tabEmployee Advance`.approver = '{user}' and `tabEmployee Advance`.workflow_state not in  ('Draft','Waiting for Verification','Rejected','Cancelled'))
	)""".format(user=user)
