...
def auto_revoke_perm():
	officiate_emp=frappe.db.sql("select name,officiate from `tabOfficiating Employee` where revoked=0 and to_date <  CURDATE()",as_dict=True)
	
	for offt in officiate_emp:
		print(offt.officiate)
		doc = frappe.get_doc("Officiating Employee", offt.name)
		doc.db_set("revoked", 1)
		if len(doc.roles) > 0:
			user = frappe.get_doc("User", frappe.db.get_value("Employee", offt.officiate, "user_id"))
			user.flags.ignore_permissions = True
			for a in doc.roles:
				user.remove_roles(str(a.role))
